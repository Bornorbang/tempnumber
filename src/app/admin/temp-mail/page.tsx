...
export default function AdminTempMailPage() {
  ...
  async function updateEmail(action: "return" | "delete", email: Email) {
    ...
    try { await fetch("/api/admin/temp-mail", { method: "POST", headers: { "Content-Type": "application/json", Authorization: "Bearer " + getToken() }, body: JSON.stringify({ action, id: email.id }) }).then(async (response) => { const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "Could not update email."); }); await load(search); }
    catch (err) { setError(err instanceof Error ? err.message : "Could not update email."); }
    finally { setUpdatingId(null); }
  }

  return <div className="max-w-6xl space-y-6">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><h1 className="text-2xl font-bold text-[var(--text-primary)]">Temporary Emails</h1><p className="mt-1 text-sm text-gray-400">{total} address{total === 1 ? "" : "es"} retained for administration</p></div><input value={search} onChange={(event) => { setSearch(event.target.value); void load(event.target.value); }} placeholder="Search email or user..." className="w-64 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-4 py-2.5 text-sm text-[var(--text-primary)] outline-none focus:border-green-500" /></header>
